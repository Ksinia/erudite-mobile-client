...
function getWinnerName(game: Game): string {
  return game.users
    .filter((user) => game.result.winner.map(String).includes(user.id.toString()))
    .map((user) => user.name)
    .join(', ');
}
...
function getTileColor(room: Game, user: User | null): string {
  if (room.phase === 'finished' && user) {
    return room.result.winner.map(String).includes(user.id.toString()) ? Colors.green : Colors.red;
  }
